...
/*
bool set_is_subset(Table* a, Table* b)
{
    bool subset = true;
    ...
    {
        for (SetIter *iter = a->begin(a); iter != a->end(a); iter = a->next(a, iter))
        {
...
Table *set_union(Table* a, Table* b, Table* c)
{
    for (SetIter *iter = a->begin(a); iter != a->end(a); iter = a->next(a, iter))
    {
        c->insert(c, iter->value);
    }
    for (SetIter *iter = b->begin(b); iter != b->end(b); iter = b->next(b, iter))
    {
        c->insert(c, iter->value);
    }
...
Table *set_difference(Table* a, Table* b, Table* c)
{
    for (SetIter *iter = b->begin(b); iter != b->end(b); iter = b->next(b, iter))
    {
...
Table *set_intersection(Table* a, Table* b, Table* c)
{
    for (SetIter *iter = a->begin(a); iter != a->end(a); iter = a->next(a, iter))
    {
        if (b->contains(b, iter->value))
