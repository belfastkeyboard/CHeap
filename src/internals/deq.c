...
static bool is_full(const size_t nmemb,
                    const size_t capacity)
{
...
static bool is_back(const size_t index,
                    const size_t back)
{
    return index == back;
}
                                 ...
                                 const bool forward)
{
    if (control->block_count >= control->capacity)
    {
        ...
        const size_t f_index = (control->capacity) / 2 - (control->block_count) / 2;

        const size_t f = f_index;
        const size_t b = f + (control->block_count) - 1;

        struct Block *blocks = calloc(control->capacity,
                                      sizeof(struct Block));
                                ...
                                const size_t size)
{
    ...
    memcpy(block->array + (arr_cap - block->nmemb) * size,
           value,
           size);
}
                          ...
                          size_t *nmemb)
{
    ...
    if (index < mid)
    {
        if (!has_space_at_front(f))
        {
            ...
            memcpy(control->blocks[1].array + index * size,
                   value,
                   size);
        }
        else
        {
            ...
            memcpy(block.array + (index - 1) * size,
                   value,
                   size);
        }
                            ...
                            size_t *nmemb)
{
    ...
    if (index > mid)
    {
        ...
        if (is_full(back_block->nmemb,
                    arr_cap))
        {
            void *last_elem = back_block->array + (back_block->nmemb - 1) * size;
    ...
    else
    {
                    ...
                    arr_cap))
        {
            ...
            memcpy(front_block->array + (arr_cap - front_block->nmemb - 1) * size,
                   first_elem,
                   size);
...
void destroy_control_array(struct ControlArray *control,
                           const size_t front)
{
    if (control->block_count == 1)
    {
    ...
    {
        for (size_t i = front, j = 0; j < control->block_count; i = (i + 1) % control->capacity, j++)
        {
                 ...
                 const size_t arr_cap)
{
    if (control->block_count > 1)
    {
        for (size_t i = *front + 1, j = 1; j < control->block_count; i = (i + 1) % control->capacity, j++)
        {
