...
{
    //clear_hash_set(*set);
    hash_clear(&(*set)->buckets, &(*set)->keys, &(*set)->capacity, &(*set)->nmemb);
...
//
//    initialise_buckets(&set->buckets, &set->capacity);
...
//        set->nmemb++;
//    }
    hash_insert(set->k_comp, &set->buckets, &set->keys, &set->capacity, set->k_size, &set->nmemb, key);
}
...
// these can go to the generic function call
